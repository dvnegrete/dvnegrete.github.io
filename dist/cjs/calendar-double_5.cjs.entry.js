...
const CalendarDouble = class {
  constructor(hostRef) {
    ...
    this.calendarDoubleSetDate = index.createEvent(this, "dvn-calendarDoubleSetDate", 7);
    this.applicationDate = index.createEvent(this, "dvn-applicationDate", 7);
    this.rangeDate = index.createEvent(this, "dvn-rangeDate", 7);
  ...
  render() {
    return (index.h(index.Host, null, index.h("calendar-single", { typeSelection: this.typeSelectionSecondary, numberCalendar: 'secondary', calendarActive: this.calendarActive, setCalendar: this.setCalendarSecond, positionRange: this.positionRangeSecondary }), index.h("calendar-single", { typeSelection: this.typeSelectionMain, numberCalendar: 'main', calendarActive: this.calendarActive, setCalendar: this.setCalendarMain, positionRange: this.positionRangeMain })));
  }
...
const CalendarInputSelection = class {
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: 'select-date', onClick: () => this.handleClick() }, index.h("p", { class: 'day-selectet-text' }, this.placeholder), index.h("div", { class: this.showCalendarDouble ? 'box box-selected' : 'box' }, index.h("svg", { xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12", viewBox: "0 0 448 512" }, index.h("path", { d: "M152 24c0-13.3-10.7-24-24-24s-24 10.7-24 24V64H64C28.7 64 0 92.7 0 128v16 48V448c0 35.3 28.7 64 64 64H384c35.3 0 64-28.7 64-64V192 144 128c0-35.3-28.7-64-64-64H344V24c0-13.3-10.7-24-24-24s-24 10.7-24 24V64H152V24zM48 192H400V448c0 8.8-7.2 16-16 16H64c-8.8 0-16-7.2-16-16V192z" })))), index.h("double-calendar-container", { class: !this.showCalendarDouble ? 'hidden' : '' })));
  }
};
...
const CalendarSingle = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.valueCalendarSelected = index.createEvent(this, "dvn-valueCalendarSelected", 7);
  ...
  */
  ...
  daysInMonthRender() {
    return this.daysInMonth.map(day => {
      ...
      if (day === 0) {
        return index.h("li", { class: 'disabled' }, day);
      }
      else {
        return (index.h("li", { class: combinedClass, onClick: () => this.daySelectedHandler(day) }, day));
      }
    });
  }
  render() {
    return (index.h("div", { class: 'container-calendar' }, index.h("header-calendar", { "name-month": this.baseNameMonth, year: this.year, position: this.numberCalendar === 'main' ? 'right' : 'left', "name-inactive": !this.calendarActive }), index.h("div", { class: this.calendarActive ? 'day-names' : 'day-names inactive' }, this.dayNames.map(dayname => {
      return (index.h("span", null, dayname));
    })), index.h("ol", { class: 'days-in-month' }, this.daysInMonthRender())));
  }
  static get watchers() { return {
    "typeSelection": ["handlerTypeSelection"],
...
const DoubleCalendarContainer = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.showDate = index.createEvent(this, "dvn-arrayDatesSelected", 7);
    this.closeDoubleCalendar = index.createEvent(this, "dvn-closeDoubleCalendar", 7);
    this.changeCleanPeriod = index.createEvent(this, "dvn-changeCleanPeriod", 7);
    this.cleanCalendarSelection = index.createEvent(this, "dvn-cleanCalendarSelection", 7);
  ...
  renderForm() {
    return this.buildPeriods().map(date => {
      return (index.h("label", { htmlFor: `period${this.monthNames[date.month]}${date.year}` }, index.h("input", { type: "radio", name: "period", id: `period${this.monthNames[date.month]}${date.year}`, value: this.monthNames[date.month] + '-' + date.year, onChange: (event) => this.markPeriodInLabel(event, date), ref: ele => {
          if (ele) {
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: 'container' }, index.h("form", { class: 'type-selection' }, index.h("label", { htmlFor: "forDay" }, index.h("input", { type: "radio", name: 'typeSelection', id: 'forDay', onInput: () => this.handlerForTypeSelection('oneDay'), defaultChecked: true }), "Por d\u00EDa"), index.h("label", { htmlFor: "forRange" }, index.h("input", { type: "radio", name: 'typeSelection', id: 'forRange', onInput: () => this.handlerForTypeSelection('range') }), "Por rango de d\u00EDas"), index.h("label", { htmlFor: "forPeriod" }, index.h("input", { type: "radio", name: 'typeSelection', id: 'forPeriod', onInput: () => this.handlerForTypeSelection('period') }), "Por periodo")), index.h("calendar-double", { class: 'calendar', typeSelection: this.typeSelection, mainDateReceived: this.assignDate }), index.h("div", { class: this.typeSelection === 'period' ? 'period-list' : 'period-list disabled' }, index.h("img", { src: "https://github-personal-dvn.s3.us-east-2.amazonaws.com/img/right-chevron.svg", alt: "anterior", onClick: () => this.changePeriod(-1) }), index.h("form", null, this.renderForm()), index.h("img", { src: "https://github-personal-dvn.s3.us-east-2.amazonaws.com/img/right-chevron.svg", alt: "siguiente", onClick: () => this.changePeriod(1) })), index.h("p", { class: 'counter-days' }, "Dias seleccionados: ", index.h("span", null, this.countDaysSelected)), index.h("button", { class: 'go-today', onClick: () => this.goToToday() }, "Ir a hoy"), index.h("button", { class: this.buttonContinue ? 'button-continue completed' : 'button-continue', onClick: () => this.closeDoubleCalendar.emit(this.buttonContinue) }, "Continuar"))));
  }
...
const HeaderCalendar = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.nextMonthCalendar = index.createEvent(this, "dvn-nextMonthCalendar", 7);
    this.previousMonthCalendar = index.createEvent(this, "dvn-previousMonthCalendar", 7);
  ...
  render() {
    return (index.h("header", null, this.twoArrow || this.position === 'left' ?
      index.h("div", { class: this.nameInactive ? 'button-next hidden' : 'button-next', onClick: () => this.previousMonthCalendar.emit() }, index.h("img", { src: "https://github-personal-dvn.s3.us-east-2.amazonaws.com/img/left-chevron.svg", alt: "atras" }))
      : index.h("div", { class: 'empty' }), index.h("span", { class: this.nameInactive ? 'inactive' : '' }, this.nameMonth, " ", this.year), this.twoArrow || this.position === 'right' ?
      index.h("div", { class: this.nameInactive ? 'button-next hidden' : 'button-next', onClick: () => this.nextMonthCalendar.emit() }, index.h("img", { src: "https://github-personal-dvn.s3.us-east-2.amazonaws.com/img/right-chevron.svg", alt: "siguiente" }))
      : index.h("div", { class: 'empty' })));
  }
};
HeaderCalendar.style = headerCalendarCss;

exports.calendar_double = CalendarDouble;
exports.calendar_input_selection = CalendarInputSelection;
exports.calendar_single = CalendarSingle;
exports.double_calendar_container = DoubleCalendarContainer;
exports.header_calendar = HeaderCalendar;

//# sourceMappingURL=calendar-double_5.cjs.entry.js.map